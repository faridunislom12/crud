                                        ...
                                        <div class="form-group float-right">
                                            ...
                                            <div class="input-group" style="cursor: pointer">
                                                <div class="input-group-prepend">
                                                    <span @click="unit.is_active=1"
                                                          :class="`input-group-text ${ unit.is_active ? 'bg-primary text-white' : '' }`"
                                                          id="popular">Да</span>
                                                </div>
                                                <div class="input-group-append">
                                                    <span @click="unit.is_active=0"
                                                          :class="`input-group-text ${ !unit.is_active ? 'bg-primary text-white' : '' }`"
...
export default {
    ...
    methods: {
        create_record: function (id) {
                axios.post(route("units.store", id), this.unit)
                    .then(
                        (response) => {
                            this.$swal({title: response.data.message,timer: 3000})
                            this.$inertia.visit('/dashboard/units');
                        },
                        (error) => {
