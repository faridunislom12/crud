...
export default {
    ...
    mounted(){
        ...
        if (this.slider.image) {
            this.$refs.myVueDropzone.manuallyAddFile(this.slider.image['fileType'], `/storage/${this.slider.image['fileName']}`);
        }
    },
    methods: {
        create_record() {
            this.checkDropzoneFiles()
            axios.put(route("sliders.update", this.slider.id), this.slider)
                .then(
                    (response) => {
